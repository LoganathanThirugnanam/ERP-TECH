...
/********************************************************************************/      
/* Procedure    :GRTGRRCSSTrMLGRrevmRef                                         */      
/* Description  :                                                               */      
/********************************************************************************/      
/* Customer     :MODEL                                                          */      
/* Project      :MODEL                                                          */      
...
CREATE   OR ALTER procedure grtgrrcsstrmlgrrevmref                                              
 @ctxt_user                     Ctxt_User,      
...
--return      
 ...
 Select        
  ...
  grdate              '_gr_date',      
  descrp              '_gr_desc',      
  grfolder              '_gr_folderml',      
  suppname              '_gr_nameml',      
  reascode              '_gr_reason_code',      
  grno              '_gr_receipt_no',      
  RTRIM(a.paramdesc)              '_gr_ref_doc',      
  remarks              '_gr_remarks',      
  revsdate              '_gr_rev_date',      
  RTRIM(c.paramdesc)              '_gr_status',      
  suppcode              '_gr_supp_code_ml',      
  RTRIM(b.paramdesc)               '_type'      
  from Zrit_gr_rever_his with(nolock),      
  component_metadata_table a(NOLOCK),      
     component_metadata_table b(NOLOCK),      
     component_metadata_table c(NOLOCK)            
  where a.componentname      = 'GR'      
  AND  a.paramcategory      = 'CSENUM'      
  AND  a.paramtype       = 'REF_DOC'      
  AND  a.paramcode       = refdoctype      
  AND  a.langid       = @ctxt_language       
  AND  b.componentname   = 'GR'      
  AND  b.paramcategory      = 'META'      
  AND  b.paramtype       = 'DOC_TYPE'      
  AND  b.paramcode       = grtype      
  AND  b.langid       = @ctxt_language       
  AND  c.componentname      = 'GR'      
  AND  c.paramcategory      = 'META'      
  AND  c.paramtype       = 'DOC_STATUS'      
  AND  c.paramcode       = grstatus      
